...
/// </para>
/// <para>
/// <strong>Security:</strong> this provider only reads static content — script execution is intentionally
/// omitted. Skill metadata is XML-escaped before prompt embedding, and resource reads are guarded against
/// path traversal and symlink escape. Only use skills from trusted sources.
/// </para>
/// </remarks>
public sealed partial class FileAgentSkillsProvider : AIContextProvider
{
    private const string DefaultSkillsInstructionPrompt =
        ...
        3. Use `read_skill_resource` to read any scripts, references, or other files mentioned by the skill
    ...
    /// <param name="options">Optional configuration for resource allowlists and prompt customization.</param>
    ...
    public FileAgentSkillsProvider(IEnumerable<string> skillPaths, FileAgentSkillsProviderOptions? options = null, ILoggerFactory? loggerFactory = null)
    {
        ...
        this._skillsInstructionPrompt = options?.SkillsInstructionPrompt ?? DefaultSkillsInstructionPrompt;

        this._loader = new FileAgentSkillLoader(this._logger, options);
        ...
        [
            ...
            AIFunctionFactory.Create(
                ...
                description: "Reads a file associated with a skill, such as scripts, references, or assets."),
        ];
    }
