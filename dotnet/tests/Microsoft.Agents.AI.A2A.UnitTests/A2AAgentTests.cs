...
public sealed class A2AAgentTests : IDisposable
{
    ...
    [Fact]
    public async Task RunAsync_WithTaskInThreadAndTextInputResponse_LinksResponseToTaskAsync()
    {
        // Arrange
        ...
        var inputMessage = new ChatMessage(ChatRole.User, [new TextInputResponseContent("response-123", "User message in response to task")]);
    ...
    [Fact]
    public async Task RunStreamingAsync_WithContinuationToken_CallsSubscribeToTaskAsyncAsync()
    {
        // Arrange
        ...
        // Assert
        Assert.Equal("tasks/resubscribe", this._handler.CapturedJsonRpcRequest?.Method);
        Assert.Equal("task-123", this._handler.CapturedTaskIdParams?.Id);
    }

    [Fact]
    public async Task RunStreamingAsync_WithTaskInThreadAndTextInputResponse_LinksResponseToTaskAsync()
    {
        // Arrange
        ...
        // Act
        await foreach (var _ in this._agent.RunStreamingAsync(new[] { inputMessage }, thread))
        {
        }
    ...
    public async Task RunStreamingAsync_WithAgentTask_YieldsResponseUpdateAsync()
    {
        ...
        // Assert - continuation token should be set for submitted task
        Assert.NotNull(update0.ContinuationToken);
        Assert.IsType<A2AContinuationToken>(update0.ContinuationToken);
        Assert.Equal(TaskId, ((A2AContinuationToken)update0.ContinuationToken).TaskId);
    ...
    public async Task RunStreamingAsync_WithTaskStatusUpdateEvent_YieldsResponseUpdateAsync()
    {
        ...
        // Assert - continuation token should be set for working task
        Assert.NotNull(update0.ContinuationToken);
        Assert.IsType<A2AContinuationToken>(update0.ContinuationToken);
        Assert.Equal(TaskId, ((A2AContinuationToken)update0.ContinuationToken).TaskId);
    ...
    public async Task RunStreamingAsync_WithTaskArtifactUpdateEvent_YieldsResponseUpdateAsync()
    {
        ...
        // Assert - continuation token should be set for working task (artifacts indicate ongoing work)
        Assert.NotNull(update0.ContinuationToken);
        Assert.IsType<A2AContinuationToken>(update0.ContinuationToken);
        Assert.Equal(TaskId, ((A2AContinuationToken)update0.ContinuationToken).TaskId);
